...
export interface CandidateFull {
  ...
  skills: string[];
  software: string[];
  assessments: { name: string; rating: string }[];
}
...
function createApi(getToken: () => Promise<string | null>) {
  ...
  return {
    ...
    // admin (Phase 3)
    createCandidate: (body: unknown) => req<CandidateFull>("/candidates", { method: "POST", body: JSON.stringify(body) }),
    updateCandidate: (id: string, body: unknown) => req<CandidateFull>(`/candidates/${id}`, { method: "PATCH", body: JSON.stringify(body) }),
